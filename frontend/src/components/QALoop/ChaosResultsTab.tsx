      ...
      {summary && (
        <div className="grid grid-cols-4 gap-4">
          <Card className="p-4 text-center">
            ...
            <div className="text-xs text-red-500 dark:text-red-400">{t('runner.qaLoop.chaos.vulnerabilities')}</div>
          </Card>
          <Card className="p-4 text-center">
            <div className="text-2xl font-bold text-green-600 dark:text-green-400">
              {summary.blockedAttacks}
            </div>
            <div className="text-xs text-green-500 dark:text-green-400">{t('runner.qaLoop.chaos.blocked')}</div>
          </Card>
          <Card className="p-4 text-center">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
              {summary.totalAttacks}
            </div>
            <div className="text-xs text-blue-500 dark:text-blue-400">{t('runner.qaLoop.chaos.totalAttacks')}</div>
          </Card>
          <Card className="p-4 text-center">
            <div className="text-2xl font-bold text-sky-600 dark:text-sky-400">
              {summary.pagesTested}
            </div>
            <div className="text-xs text-sky-500 dark:text-sky-400">{t('runner.qaLoop.chaos.pagesTested')}</div>
          </Card>
        </div>
      )}
      ...
      {summary && summary.vulnerabilitiesFound > 0 && (
        <Card className="p-3">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-sm text-muted-foreground me-2">{t('runner.qaLoop.chaos.bySeverity')}</span>
            {summary.criticalFindings > 0 && (
              <Badge variant="outline" className="bg-red-100 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800">
                {summary.criticalFindings} {t('runner.qaLoop.chaos.critical')}
              </Badge>
            )}
            {summary.highFindings > 0 && (
              <Badge variant="outline" className="bg-orange-100 text-orange-700 border-orange-200 dark:bg-orange-900/30 dark:text-orange-300 dark:border-orange-800">
                {summary.highFindings} {t('runner.qaLoop.chaos.high')}
              </Badge>
            )}
            {summary.mediumFindings > 0 && (
              <Badge variant="outline" className="bg-yellow-100 text-yellow-700 border-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-300 dark:border-yellow-800">
                {summary.mediumFindings} {t('runner.qaLoop.chaos.medium')}
              </Badge>
            )}
            {summary.lowFindings > 0 && (
              <Badge variant="outline" className="bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800">
                {summary.lowFindings} {t('runner.qaLoop.chaos.low')}
              </Badge>
            )}
          </div>
        </Card>
      ...
      <Tabs value={filter} onValueChange={v => setFilter(v as typeof filter)}>
        <TabsList>
          <TabsTrigger value="all">
            {t('runner.qaLoop.chaos.all', { count: results.length })}
          </TabsTrigger>
          <TabsTrigger value="vulnerable">
            {t('runner.qaLoop.chaos.vulnerabilitiesTab', { count: vulnerabilities.length })}
          </TabsTrigger>
          <TabsTrigger value="blocked">
            {t('runner.qaLoop.chaos.blockedTab', { count: results.filter(r => r.result === 'blocked').length })}
          </TabsTrigger>
        </TabsList>
          ...
          <div className="max-h-96 overflow-y-auto space-y-2">
            {isRunning && results.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">
                ...
                {t('runner.qaLoop.chaos.running')}
              </div>
            )}

            {!isRunning && results.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">
                {t('runner.qaLoop.chaos.empty')}
              </div>
            )}
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground truncate max-w-xs">
                        {result.pageUrl ? new URL(result.pageUrl).pathname : t('runner.qaLoop.chaos.unknownPage')}
                      </div>
                    </div>
                  </div>
                ...
                {expandedResult === result.id && (
                  <div className="mt-3 pt-3 border-t border-border space-y-3">
                    <div>
                      <div className="text-xs font-medium text-muted-foreground mb-1">{t('runner.qaLoop.chaos.payloadUsed')}</div>
                    ...
                    {result.elementSelector && (
                      <div>
                        <div className="text-xs font-medium text-muted-foreground mb-1">{t('runner.qaLoop.chaos.element')}</div>
                    ...
                    {result.reproductionSteps.length > 0 && (
                      <div>
                        <div className="text-xs font-medium text-muted-foreground mb-1">{t('runner.qaLoop.chaos.reproductionSteps')}</div>
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-muted-foreground">
                        {t('runner.qaLoop.chaos.confidence', { value: Math.round(result.confidence * 100) })}
                      </span>
                      {result.pageUrl && (
                        <a
                          href={result.pageUrl}
                        ...
                        >
                          {t('runner.qaLoop.chaos.openPage')} <ExternalLink className="h-3 w-3 rtl:scale-x-[-1]" />
                        </a>
                      )}
                    </div>
                    ...
                    {result.evidenceScreenshot && (
                      <div>
                        <div className="text-xs font-medium text-muted-foreground mb-1">{t('runner.qaLoop.chaos.evidence')}</div>
                        <img
                          src={result.evidenceScreenshot}
                          alt={t('runner.qaLoop.chaos.evidenceScreenshot')}
