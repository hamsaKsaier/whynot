...
export const ChaosResultsTab: React.FC<ChaosResultsTabProps> = ({
  ...
  const getSeverityColor = (severity?: string) => {
    switch (severity) {
      ...
      case 'medium': return 'bg-yellow-900/200 text-black';
      case 'low': return 'bg-blue-900/200 text-white';
      ...
      {summary && summary.vulnerabilitiesFound > 0 && (
        <div className="flex items-center gap-2 p-3 bg-slate-900 rounded-lg">
          ...
          {summary.mediumFindings > 0 && (
            <span className="px-2 py-1 rounded text-xs bg-yellow-900/200 text-black">
              {summary.mediumFindings} Medium
            </span>
          )}
          {summary.lowFindings > 0 && (
            <span className="px-2 py-1 rounded text-xs bg-blue-900/200 text-white">
