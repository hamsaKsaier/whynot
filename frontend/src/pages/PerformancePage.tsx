...
import { startPerfRun, stopPerfRun, getPerfRuns, getPerfRun } from '../services/perf-api';
...
export const PerformancePage: React.FC = () => {
  ...
  const loadPastRuns = useCallback(async () => {
    try {
      const { runs } = await getPerfRuns({ limit: 10 });
      setPastRuns(runs);
    } catch {
      // ignore — not critical
    }
  ...
  const handleSelectPastRun = async (run: PerfRun) => {
  ...
  const pastRunSummary = selectedPastRun ? {
    ...
    failedRequests: selectedPastRun.failed_requests || 0,
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6">
          ...
          <div>
            ...
            {pastRuns.length > 0 && (
              <div className="mt-6">
                <h3 className="text-sm font-medium text-slate-300 mb-3">Previous Runs</h3>
